...
public class Solution {
	/*static int programmerStrings(String string) {
		...
		for (int i = 0; i < string.length(); i++) {
			...
			} else if (compariableChar == 'r' && rCounter < 4) {
				tempString.append(compariableChar);
			} else if (compariableChar == 'o' && !tempString.toString().contains("o")) {
				tempString.append(compariableChar);
			} else if (compariableChar == 'g' && !tempString.toString().contains("p")) {
				tempString.append(compariableChar);
			} else if (compariableChar == 'a' && !!tempString.toString().contains("a")) {
				tempString.append(compariableChar);
			} else if (compariableChar == 'm' && mCounter < 3) {
				tempString.append(compariableChar);
